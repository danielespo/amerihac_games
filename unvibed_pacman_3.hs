...
frameDelay :: Int
frameDelay = 100000  -- 100ms per frame 100000
...
movePacman game@Game{..} = game { pacmanPos = wrappedPos, pacmanDir = newDir }
  where
    ...
    -- Apply wraparound for tunnel
    wrappedPos = wrapPosition newPos
...
moveGhost game ghost@Ghost{..} = ghost { ghostPos = wrappedPos, ghostDir = newDir }
  where
